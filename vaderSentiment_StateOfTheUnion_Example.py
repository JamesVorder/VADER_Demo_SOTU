...
if __name__ == '__main__':
    ...
    out_file = open("C:\\Users\\jvorderb\\Desktop\\Open Source Sentiment Analysis Demo\\out.csv", "w") #gotta store results somewhere
    ...
    print("For this fun little diddy, we're going to analyze state of the union addresses over time.")
    import os
    for filename in os.listdir(target_dir):
        if filename.endswith(".txt"): #only read the text files
            in_file = open(target_dir + filename, "r")
            stateOfTheUnion = in_file.read()
            in_file.close() #close this baby up
            from nltk import tokenize
            sentence_list = tokenize.sent_tokenize(stateOfTheUnion)
            documentSentiments=0.0
            documentSentiments_pos=0.0
            documentSentiments_neg=0.0
            for sentence in sentence_list:
                vs = analyzer.polarity_scores(sentence)
                documentSentiments += vs["compound"]
                documentSentiments_pos += vs["pos"]
                documentSentiments_neg += vs["neg"]
            print("AVERAGE SENTIMENT FOR DOCUMENT: \t" + str(round(documentSentiments/len(sentence_list), 4)))
            out_file.write(filename[0:4] + "," + str(round(documentSentiments/len(sentence_list), 4)) + "," + str(round(documentSentiments_pos/len(sentence_list),4)) + "," + str(round(documentSentiments_neg/len(sentence_list),4)*-1) + "\n")
    out_file.close()
    print("\n\n Demo Done!")
